...
def export_report_bundle(result: ScanResult, output_dir: Path) -> Path:
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    _write_summary_csv(result, output_dir / "summary.csv")
    _write_files_csv(result, output_dir / "files.csv")
    _write_duplicates_csv(result, output_dir / "duplicates.csv")
    _write_risks_csv(result, output_dir / "risks.csv")
    _write_errors_csv(result, output_dir / "errors.csv")
...
def _write_files_csv(result: ScanResult, path: Path) -> None:
    rows = []
    ...
    _write_csv(
        path,
        ["文件名", "完整路径", "扩展名", "大小字节", "大小", "创建时间", "修改时间", "隐藏", "空文件", "风险", "原因", "Hash"],
        rows,
    )


def _write_duplicates_csv(result: ScanResult, path: Path) -> None:
    rows = []
    ...
    _write_csv(path, ["组号", "文件名", "完整路径", "大小字节", "大小", "组内文件数", "Hash", "组可节省字节", "组可节省空间"], rows)


def _write_risks_csv(result: ScanResult, path: Path) -> None:
    ...
    _write_csv(path, ["风险", "文件名", "完整路径", "扩展名", "大小字节", "大小", "修改时间", "原因"], rows)


def _write_errors_csv(result: ScanResult, path: Path) -> None:
    rows = [[str(error.path), error.message] for error in result.errors]
    _write_csv(path, ["路径", "错误"], rows)
...
def _build_html_report(result: ScanResult) -> str:
    ...
    directory_items = _directory_size_distribution(result)
...
<main>
  ...
  <div class="muted">扫描目录：{_escape(str(summary.root_path))}</div>
...
def _risk_preview_table(result: ScanResult) -> str:
    ...
    for record in records:
        rows.append(
            ...
            f'<td class="path">{_escape(str(record.path))}</td>'
            f"<td>{_escape(_format_size(record.size))}</td>"
            f"<td>{_escape(_format_risk_reasons(record.risk_reasons))}</td>"
            "</tr>"
        )
    return "<table><thead><tr><th>风险</th><th>文件名</th><th>完整路径</th><th>大小</th><th>原因</th></tr></thead><tbody>" + "".join(rows) + "</tbody></table>"
...
def _directory_size_distribution(result: ScanResult) -> list[tuple[str, int]]:
    sizes: dict[str, int] = {}
    for record in result.records:
        path = str(record.parent)
