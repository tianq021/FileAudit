...
- 当前“忽略目录”只表示跳过目录名匹配的整个目录，不是通用文件跳过规则。
...
原因：扫描记录本身相对可控，但 `QTableWidgetItem` 数量会按“行数 × 列数”暴涨。

当前处理：

- 扫描结果完整保留在 `ScanResult` 中。
- UI 表格只渲染前 `MAX_TABLE_ROWS = 5000` 行。
- 导出报告仍使用完整扫描结果。
...
当前扫描规则里，“忽略目录”只处理目录名，例如 `.git`、`node_modules`、`.venv`。后续可以扩展出独立的 `SkipOptions` 或 `SkipRules`，和风险检测规则分开。
...
跳过规则 = 文件不进入扫描结果
风险规则 = 文件进入结果，但被标记为风险
白名单规则 = 文件进入结果，但不因指定扩展名被标记风险
```

如果以后支持“跳过大文件”，需要明确它是否只跳过 Hash，还是整个文件记录都不生成。更推荐第一步做“跳过大文件 Hash”，第二步再做“完全跳过大文件扫描”。
